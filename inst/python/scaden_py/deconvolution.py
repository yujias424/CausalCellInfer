...
def ScadenDeconvolution(necessary_data, real_bulk, sep='\t', sparse=True,
                        batch_size=128, epochs=128, samplenum = 5000, n = 500,
                        cell_type="None", generate_sim_method = "scaden", cut_variance = True, id_col = "patient_id",
                        add_noise = False, noise_level = 0.01, add_realdat = False, real_dat = None, markergene = None):
    if type(necessary_data) is str:
        postfix = necessary_data.split('.')[-1]
        if postfix == 'txt' or postfix == 'gz':
            if generate_sim_method == "tape":
                simudata = generate_simulated_data_tape(sc_data=necessary_data, samplenum = samplenum, sparse=sparse, n = n,
    ...
    train_x, train_y, test_x, genename, celltypes, samplename = \
        ProcessInputData(simudata, real_bulk, sep=sep, datatype='counts', cut_variance = cut_variance, markergene = markergene)
