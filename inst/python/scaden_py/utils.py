...
def counts2CPM(counts, genelen):
    ...
    # transformation
    totalreads = counts.sum(axis=1)
    counts = counts * 1e6 / (totalreads.reshape(-1, 1))
    counts = pd.DataFrame(counts, columns=inter, index=samplename)
    return counts
...
def ProcessInputData(train_data, test_data, sep=None, datatype='TPM', variance_threshold=0.98,
    ...
    try:
        ...
        train_x = pd.DataFrame(train_data.X.todense(), columns=train_data.var.index)
        print(train_x.shape)
        print(train_x)
    ...
    if markergene != None:
        markergene = test_x.columns.intersection(markergene)
        test_x = test_x.loc[:, markergene]
    ...
    test_x.index = test_x.index.astype(str)
